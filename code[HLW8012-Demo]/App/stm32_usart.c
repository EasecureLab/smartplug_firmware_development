...
void USART1_IRQHandler(void)                    //串口1中断服务程序
{
    ...
    {
        recData = USART_ReceiveData(USART1);//(USART1->DR); //读取接收到的数据
        //sunmQueuePush(&sunmRecQ, recData);
        //sunmRecPush(&app_tProt, recData);                             // 接收数据--举例一
        //sunmRecPush(&app_tProt, recData);                             // 接收数据--举例一
    }
    #ifdef OS_TICKS_PER_SEC     //如果时钟节拍数定义了,说明要使用ucosII了.
    OSIntExit();
...
void txByte(USART_TypeDef *USARTx, unsigned char byte)
{
    ...
    USARTx->DR = ((uint16_t)byte & (uint16_t)0x01FF);
}
...
void UART4_IRQHandler(void)                 //串口1中断服务程序
{
    ...
    {
        recData = USART_ReceiveData(UART4);//(USART1->DR);  //读取接收到的数据
        //sunmQueuePush(&sunmRecQ, recData);
        //        sunmRecPush(&app_tProt, recData);                             // 接收数据--举例一
    }
    #ifdef OS_TICKS_PER_SEC     //如果时钟节拍数定义了,说明要使用ucosII了.
